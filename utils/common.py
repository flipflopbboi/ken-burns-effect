...
def process_inpaint(tensorShift, objectCommon, moduleInpaint, dblFocal):

	# if two networks are used for inpainting compute the color inpainting with the first one and disparity inpainting with the 2nd
	if type(moduleInpaint) == list: 
		objectInpaintedColor = moduleInpaint[0].pointcloud_inpainting(objectCommon['tensorRawImage'], 
														objectCommon['tensorRawDisparity'], 
														tensorShift, 
														objectCommon, dblFocal)
		objectInpaintedDepth = moduleInpaint[1].pointcloud_inpainting(objectCommon['tensorRawImage'], 
														objectCommon['tensorRawDisparity'], 
														tensorShift, 
														objectCommon, dblFocal)

		objectInpaint = {'tensorExisting': objectInpaintedColor['tensorExisting'],
						 'tensorImage': objectInpaintedColor['tensorImage'],
						 'tensorDisparity': objectInpaintedDepth['objectInpaintedDepth']}
	else:
		objectInpainted = moduleInpaint.pointcloud_inpainting(objectCommon['tensorRawImage'], 
															objectCommon['tensorRawDisparity'], 
															tensorShift, 
															objectCommon, dblFocal)

	objectInpainted['tensorDepth'] = (dblFocal * objectCommon['dblBaseline']) / (objectInpainted['tensorDisparity'] + 0.0000001)
	...
	objectInpainted['tensorPoints'] = depth_to_points(objectInpainted['tensorDepth'] * objectInpainted['tensorValid'], dblFocal)
...
def process_shift(objectSettings, objectCommon, dblFocal=None):
	...
	dblClosestFromX = ((dblClosestFromU - (objectCommon['intWidth'] / 2.0)) * dblClosestDepth) / dblFocal
	dblClosestFromY = ((dblClosestFromV - (objectCommon['intHeight'] / 2.0)) * dblClosestDepth) / dblFocal
	dblClosestToX = ((dblClosestToU - (objectCommon['intWidth'] / 2.0)) * dblClosestDepth) / dblFocal
	dblClosestToY = ((dblClosestToV - (objectCommon['intHeight'] / 2.0)) * dblClosestDepth) / dblFocal
...
def process_autozoom(objectSettings, objectCommon):
	numpyShiftU = np.linspace(-objectSettings['dblShift'], objectSettings['dblShift'], 16)[None, :].repeat(16, 0)
	numpyShiftV = np.linspace(-objectSettings['dblShift'], objectSettings['dblShift'], 16)[:, None].repeat(16, 1)
...
def process_kenburns(objectSettings, objectCommon, moduleInpaint):
	numpyOutputs = []
	...
	if 'boolInpaint' not in objectSettings or objectSettings['boolInpaint'] == True:
		...
		for dblStep in [ 0.0, 1.0 ]:
			...
			tensorShift = process_shift({
				...
				'dblDepthFrom': dblDepthFrom,
				'dblDepthTo': dblDepthTo
			}, objectCommon, currentFocal)[1]

			tensorRender, tensorExisting = render_pointcloud(objectCommon['tensorInpaPoints'] + tensorShift, 
                                                 torch.cat([ objectCommon['tensorInpaImage'], objectCommon['tensorInpaDisparity'] ], 1).view(1, 4, -1), 
	...
	for dblStep in objectSettings['dblSteps']:
		...
		tensorPoints = process_shift({
			...
			'dblDepthFrom': dblDepthFrom,
			'dblDepthTo': dblDepthTo
		}, objectCommon, currentFocal)[0]

		tensorRender, tensorExisting = render_pointcloud(tensorPoints, torch.cat([ objectCommon['tensorInpaImage'], 
														objectCommon['tensorInpaDepth'] ], 1).view(1, 4, -1), 
														objectCommon['intWidth'], 
														objectCommon['intHeight'], 
														currentFocal, 
														objectCommon['dblBaseline'])
...
def generate_mask(tensorInput, tensorShift, intWidth, intHeight, dblFocal, dblBaseline):
	...
	tensorZee = tensorInput.new_zeros([ tensorInput.size(0), 1, intHeight, intWidth ]).fill_(1000000.0)
	tensorMasks = torch.zeros(tensorInput.size(0), 1, intHeight * intWidth).to(device)
	tensorIds = torch.ones(tensorInput.size(0), intHeight, intWidth).to(device) * -1 
	n = tensorInput.size(0) * tensorInput.size(2)
	launch_kernel('kernel_pointrender_updateZee', preprocess_kernel('''
		extern "C" __global__ void kernel_pointrender_updateZee(
			const int n,
			...
			if ((dblNorthwest >= dblNortheast) & (dblNorthwest >= dblSouthwest) & (dblNorthwest >= dblSoutheast)) {
				if ((intNorthwestX >= 0) & (intNorthwestX < SIZE_3(zee)) & (intNorthwestY >= 0) & (intNorthwestY < SIZE_2(zee))) {
					if (zee[OFFSET_4(zee, intSample, 0, intNorthwestY, intNorthwestX)] > dblError) {
						atomicMin(&zee[OFFSET_4(zee, intSample, 0, intNorthwestY, intNorthwestX)], dblError);
						atomicMax(&masks[OFFSET_3(masks, intSample, 0, intPoint)], one_val);
						int pid = atomicExch(&id_memory[OFFSET_3(id_memory, intSample, intNorthwestY, intNorthwestX)], intPoint);
						if (pid > 0){
							atomicMin(&masks[OFFSET_3(masks, intSample, 0, pid)], zero_val);
						}
						
					} else {
						atomicMin(&masks[OFFSET_3(masks, intSample, 0, intPoint)], zero_val);
					}
				...
				if ((intNortheastX >= 0) & (intNortheastX < SIZE_3(zee)) & (intNortheastY >= 0) & (intNortheastY < SIZE_2(zee))) {
					if (zee[OFFSET_4(zee, intSample, 0, intNortheastY, intNortheastX)] > dblError) {
						atomicMin(&zee[OFFSET_4(zee, intSample, 0, intNortheastY, intNortheastX)], dblError);
						atomicMax(&masks[OFFSET_3(masks, intSample, 0, intPoint)], one_val);
						int pid = atomicExch(&id_memory[OFFSET_3(id_memory, intSample, intNortheastY, intNortheastX)], intPoint);
						if (pid > 0){
							atomicMin(&masks[OFFSET_3(masks, intSample, 0, pid)], zero_val);
						}
					} else {
				...
				if ((intSouthwestX >= 0) & (intSouthwestX < SIZE_3(zee)) & (intSouthwestY >= 0) & (intSouthwestY < SIZE_2(zee))) {
					if (zee[OFFSET_4(zee, intSample, 0, intSouthwestY, intSouthwestX)] > dblError) {
						atomicMin(&zee[OFFSET_4(zee, intSample, 0, intSouthwestY, intSouthwestX)], dblError);
						atomicMax(&masks[OFFSET_3(masks, intSample, 0, intPoint)], one_val);
						int pid = atomicExch(&id_memory[OFFSET_3(id_memory, intSample, intSouthwestY, intSouthwestX)], intPoint);
						if (pid > 0){
							atomicMin(&masks[OFFSET_3(masks, intSample, 0, pid)], zero_val);
						}
					} else {
				...
				if ((intSoutheastX >= 0) & (intSoutheastX < SIZE_3(zee)) & (intSoutheastY >= 0) & (intSoutheastY < SIZE_2(zee))) {
					if (zee[OFFSET_4(zee, intSample, 0, intSoutheastY, intSoutheastX)] > dblError) {
						atomicMin(&zee[OFFSET_4(zee, intSample, 0, intSoutheastY, intSoutheastX)], dblError);
						atomicMax(&masks[OFFSET_3(masks, intSample, 0, intPoint)], one_val);
						int pid = atomicExch(&id_memory[OFFSET_3(id_memory, intSample, intSoutheastY, intSoutheastX)], intPoint);
						if (pid > 0){
							atomicMin(&masks[OFFSET_3(masks, intSample, 0, pid)], zero_val);
						}
					} else {
	...
	''', {
		...
		grid=tuple([ int((n + 512 - 1) / 512), 1, 1 ]),
		block=tuple([ 512, 1, 1 ]),
		args=[ n, tensorInput.data_ptr(), tensorMasks.data_ptr(), tensorZee.data_ptr(), tensorIds.data_ptr()],
		stream=Stream
	)

	tensorMasks =  spatial_filter(tensorMasks.view(-1,1,intHeight,intWidth), 'median-5')
	return tensorMasks
